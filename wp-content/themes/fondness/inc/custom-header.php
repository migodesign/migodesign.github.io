 ...
 *
 * @uses fondness_header_style()
 */
function fondness_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'fondness_custom_header_args', array(
		'default-image'          => '%s/assets/uploads/header-image.jpg',
		'default-text-color'     => '000',
		'width'                  => 1200,
		'height'                 => 528,
		'flex-height'            => true,
		'wp-head-callback'       => 'fondness_header_style',
	) ) );
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'fondness' ),
		),
	) );
}
add_action( 'after_setup_theme', 'fondness_custom_header_setup' );

if ( ! function_exists( 'fondness_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see fondness_custom_header_setup().
	 */
	function fondness_header_style() {
		...
		wp_add_inline_style( 'fondness-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'fondness_header_style', 10 );